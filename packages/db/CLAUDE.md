...
`apps/api/src/prisma/prisma.service.ts` imports `createPrismaClient` from `@composer-ai/db` and wraps it as a NestJS injectable service. All database access goes through `PrismaService` — never import PrismaClient directly in feature modules.
